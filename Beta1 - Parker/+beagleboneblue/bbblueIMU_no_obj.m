classdef bbblueIMU_no_obj < matlab.System ...
    ...
    methods(Access = protected)
        function [conf, data] = setupImpl(~)
            ...
            % data structure declaration
                ...
                conf = coder.ceval('rc_set_imu_config_to_defaults'); %, coder.ref(conf))
                
                % Initialize the IMU with data and conf pointers
                coder.ceval('rc_initialize_imu', coder.ref(data), coder.ref(conf))
            end
        end
        
        function [accel, gyro, mag, temp] = stepImpl(data)
            % Implement algorithm. Calculate y as a function of input u and
            %  discrete states.
            if coder.target('Rtw')
                % Read data                
                coder.ceval('rc_read_accel_data', coder.ref(data));
                coder.ceval('rc_read_gyro_data', coder.ref(data));
                coder.ceval('rc_read_mag_data', coder.ref(data));
                coder.ceval('rc_read_imu_temp', coder.ref(data));
                % Output data
                accel = data.accel;
                gyro = data.gyro;
                mag = data.mag;
