classdef bbblueIMU_obj < matlab.System ...
    ...
    methods(Access = protected)
        function setupImpl(obj)
            ...
            % config structure declaration
                ...
                obj.conf.accel_fsr = coder.opaque('rc_accel_fsr_t', 'A_FSR_4G'); % AFS_2G, AFS_4G, AFS_8G, AFS_16G
                obj.conf.gyro_fsr = coder.opaque('rc_gyro_fsr_t', 'G_FSR_1000DPS');  % GFS_250,GFS_500,GFS_1000,GFS_2000
                % internal low pass filter constants
                obj.conf.gyro_dlpf = coder.opaque('rc_gyro_dlpf_t', 'GYRO_DLPF_92');
                obj.conf.accel_dlpf = coder.opaque('rc_accel_dlpf_t', 'ACCEL_DLPF_92');
                % magnetometer use is optional
                obj.conf.enable_magnetometer = coder.opaque('int','0'); % 0 or 1
                % DMP settings, only used with DMP interrupt
                obj.conf.dmp_sample_rate = coder.opaque('int', '100');
                obj.conf.orientation = coder.opaque('rc_imu_orientation_t', 'ORIENTATION_Z_UP'); %orientation matrix
                obj.conf.compass_time_constant = coder.opaque('float', '5.0'); 	% time constant for filtering fused yaw
                obj.conf.dmp_interrupt_priority = coder.opaque('int', 'sched_get_priority_max(SCHED_FIFO)-1'); % scheduler priority for handler
                obj.conf.show_warnings = coder.opaque('int', '0');	% set to 1 to enable showing of rc_i2c_bus warnings
                
                % Indicate struct name in header
                coder.cstructname(obj.conf, 'rc_imu_config_t', 'extern', 'HeaderFile', 'roboticscape.h');
                
            % data structure declaration
                % last read sensor values in real units
                obj.data.accel = coder.opaque('float');	% units of m/s^2
                obj.data.gyro = coder.opaque('float');	% units of degrees/s
                obj.data.mag = coder.opaque('float');	% units of uT
                obj.data.temp = coder.opaque('float');  % units of degrees Celsius

                % 16 bit raw adc readings from each sensor
                obj.data.raw_gyro = coder.opaque('int16_t');	
                obj.data.raw_accel = coder.opaque('int16_t');

                % FSR-derived conversion ratios from raw to real units
                obj.data.accel_to_ms2 = coder.opaque('float');	% to m/s^2
                obj.data.gyro_to_degs = coder.opaque('float');	% to degrees/s
                ...
                obj.data.dmp_quat = coder.opaque('float');		% normalized quaternion
                obj.data.dmp_TaitBryan = coder.opaque('float');	% radians pitch/roll/yaw X/Y/Z
                ...
                obj.data.fused_quat = coder.opaque('float');		% normalized quaternion
                obj.data.fused_TaitBryan = coder.opaque('float');	% radians pitch/roll/yaw X/Y/Z
                obj.data.compass_heading = coder.opaque('float');		% heading filtered with gyro and accel data
                obj.data.compass_heading_raw = coder.opaque('float');	% heading in radians from magnetometer
                
                % Indicate struct name in header
                coder.cstructname(obj.data, 'rc_imu_data_t', 'extern', 'HeaderFile', 'roboticscape.h');
                
                % Set configuration parameters to default
                obj.conf = coder.ceval('rc_set_imu_config_to_defaults');
